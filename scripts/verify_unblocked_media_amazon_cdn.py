...
# 100% UNBLOCKED DIRECT m.media-amazon.com HIGH-RES PRODUCT PHOTOS
DIRECT_MEDIA_AMAZON_CATALOG = [
    {
...
def generate_rss(deals):
    try:
        ...
        ET.SubElement(channel, "title").text = "Deals Hub — Unblocked Real Deals & Discounts"
...
def main():
    ...
    passed = 0
    for p in DIRECT_MEDIA_AMAZON_CATALOG:
    ...
    print(f"\nSCORE: {passed}/{len(DIRECT_MEDIA_AMAZON_CATALOG)} PASSED!")

    if passed == len(DIRECT_MEDIA_AMAZON_CATALOG):
        with open(DEALS_JSON_PATH, "w", encoding="utf-8") as f:
            json.dump(DIRECT_MEDIA_AMAZON_CATALOG, f, indent=2, ensure_ascii=False)
        generate_rss(DIRECT_MEDIA_AMAZON_CATALOG)
        print("PERFECT! Saved 16 unblocked real product photos & functioning links to deals.json!")
